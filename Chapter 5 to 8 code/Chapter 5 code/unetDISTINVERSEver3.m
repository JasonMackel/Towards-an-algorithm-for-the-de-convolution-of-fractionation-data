...
function out = unetDISTINVERSEver3(s,d)
...
%Tstar of particle size s
ts = tSTARver3(s);

%situation where particle velocity is positive
if (Pv >= 0)
    if (0 < d) && (d < t0*Pv)
        z = d/Pv;
    else
        % Quadratic formula of inverse distance.
        z = (lambda*Uloc*t0 - Pv + sqrt((Pv)^2 - 2*lambda*Uloc*t0*Pv + 2*lambda*Uloc*d))/(lambda*Uloc);
    end
else
    % The situation where particle velocity is less than 0.
