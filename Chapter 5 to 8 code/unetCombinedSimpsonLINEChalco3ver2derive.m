function out = unetCombinedSimpsonLINEChalco3ver2derive(a,b,j1,j2,factor,k,degree)
...
if degree == 1
    Result = linearPieceWiseChalco(x);
else
    Result = uniformPieceWiseChalco(x);
end
...
% f = @(x) unetSuperAdapt(x,(j-1)*10,10*j,k);
 f = @(x) unetSuperKernelChalcoDerive(x,j1*factor,j2*factor,k);
