function out = unetCombinedSimpsonLINE3ver3derive(a,b,j1,j2,factor,k,degree)
...
if degree == 1
    Result = linearPieceWiseSilica(x);
else
    Result = uniformPieceWiseSilica(x);
end
...
% f = @(x) unetSuperAdapt(x,(j-1)*10,10*j,k);
 f = @(x) unetSuperKernelver2Derive(x,j1*factor,j2*factor,k);
