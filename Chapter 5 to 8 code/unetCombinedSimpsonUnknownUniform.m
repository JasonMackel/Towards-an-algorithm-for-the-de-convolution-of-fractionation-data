function out = unetCombinedSimpsonUnknownUniform(a,b,s1,s2,j,k,factor)
...
%Result = piecewiseUniformModel(x);

uniform = 1/(s2 - s1);

 f = @(x) unetSuperKernelver2(x,(j-1)*factor,j*factor,k);
...
XI0 =uniform*f(a*2000) + uniform*f(b*2000);
XI1 = 0;
XI2 = 0;
...
for i = 2:length(x)-1
    ...
    if (mod(i,2) == 0)
        XI1 = XI1 + uniform*f(x(i)*2000);
    else
        XI2 = XI2 + uniform*f(x(i)*2000);
    end
