...
function out = unetDISTINVERSEver2(s,d)
...
%Unet = (t -ts)/2*(lambda*U*t - Utincline);

%z = ((lambda.*U.*ts + Utincline) + sqrt((lambda.*U.*ts + Utincline).^2 -4.*lambda.*U.*(Utincline.*ts - 2.*d)))/(2.*lambda.*U);

%z = ((2.*Utincline) + sqrt((2.*Utincline).^2 -4.*lambda.*U.*(Utincline.*ts - 2.*d)))/(2.*lambda.*U);

%z = ((2.*Utincline) + sqrt(8.*lambda.*U.*d))/(2.*lambda.*U);

% as lambda*U*ts - Utincline = 0;

%z = ts +  sqrt(8.*lambda.*U.*d)/(2.*lambda.*U);

z = ts + sqrt((2.*d)/(lambda.*U));
